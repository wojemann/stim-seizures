...
def evaluate_model(param_dict):
    ...
    dropout = param_dict['dropout'] if param_dict['dropout'] is not None else 'None'
    weights_str = '_'.join([str(w) for w in param_dict['weights']]) if param_dict['weights'] is not None else 'None'
    num_stacks = param_dict['num_stacks'] if param_dict['num_stacks'] is not None else 'None'
    duration = param_dict['duration']
    
    filename = f"{param_dict['patient']}_{duration}_{mdl_str}_{param_dict['sequence_length']}_{num_layers}_{dropout}_{weights_str}_{num_stacks}.pkl"
    ...
    # for x_len in [90]:
        ...
        # for sequence_length in [128]:
            for num_layers in [1, 2]:
                for dropout in [0.1]:
                    # Generate weight combinations for current num_layers
                    weight_options = [0.3, 0.5, 0.8, 1,2]
                    weight_combinations = list(product(weight_options, repeat=num_layers))
                    
                    for weights in weight_combinations:
                        # Calculate hidden sizes by applying weights to n_channels
                        hidden_sizes = [int(weight * n_channels*sequence_length) for weight in weights]
...
# res_series_list = pqdm(param_dict_list, evaluate_model, n_jobs=4)
...
# res_series_list.to_csv(ospj(prodatapath,'res_series_list_mindd.csv'))
