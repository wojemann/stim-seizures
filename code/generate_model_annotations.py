...
def main():
    ...
    for _,row in pbar:
        ...
        for i,(_,sz_row) in enumerate(qbar):
            ...
            approx_time = annot_row.approximate_onset.item()
            ...
            for final_thresh in np.linspace(0,1,int(1/0.05)+1):
                predicted_channels['Seizure_ID'] = annot_row.Seizure_ID.item()
                predicted_channels['Patient'] = annot_row.Patient.item()
                predicted_channels['iEEG_ID'] = annot_row.iEEG_ID.item()
                predicted_channels['approximate_onset'] = annot_row.approximate_onset.item()
                predicted_channels['ueo_time_consensus'] = annot_row.ueo_time_consensus.item()
                predicted_channels['all_chs'] = annot_row.all_chs.item()
                ...
                first_detect = np.argmax(sz_prob[:,(onset_index-10):]>final_thresh,axis=1)
                ...
                mdl_ueo_ch_loose = np.array([s.split("-") for s in mdl_ueo_ch_bp]).flatten()
                ...
                mdl_sec_ch_loose = np.array([s.split("-") for s in mdl_sec_ch_bp]).flatten()
