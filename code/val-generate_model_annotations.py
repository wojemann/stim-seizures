                            ...
                            for _,sz_row in qbar:
                                ...
                                for mdl_str in mdl_strs:
                                    ...
                                    if (epochs == 100) & (mdl_str == 'LSTM'):
                                        prob_path = f"pretrain_probability_matrix_nosmooth_nepochs-{epochs}_mdl-{mdl_str}_fs-{clf_fs}_montage-{montage}_task-{task}_run-{run}.pkl"
                                    else:
                                    ...
                                    if movdata=='prob':
                                        if movtype == 'mean':
                                            sz_prob = sc.ndimage.uniform_filter1d(sz_prob,size=movwin,mode='nearest',axis=1,origin=0)
                                        if movtype == 'med':
                                            sz_prob = sc.ndimage.median_filter(sz_prob,size=movwin,mode='nearest',axes=1,origin=0)
                                    ...
                                    for final_thresh in np.linspace(0,4,750):
                                        ...
                                        if movdata == 'clf':
                                            if movtype == 'mean':
                                                sz_clf_final = sc.ndimage.uniform_filter1d(sz_clf,size=movwin,mode='nearest',axis=1,origin=0)
                                            if movtype == 'med':
                                                sz_clf_final = sc.ndimage.median_filter(sz_clf,size=movwin,mode='nearest',axes=1,origin=0)
                                        else:
                                            sz_clf_final = sz_clf
