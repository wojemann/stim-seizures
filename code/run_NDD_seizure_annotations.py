...
def run_model_task(params: tuple) -> list:
    ...
    try:
        #TODO: add back in the precheck functionality after the first run
        ...
        #     model_paths[model_class] = {}
        #     for forecast in all_forecasts:
        #         # Create output directory and path using onset_run as task name
        #         out_dir = ospj(prodatapath, 'sz_prob', patient)
        #         out_path = ospj(out_dir, f"{patient}_task-ictal{onset_run}_run-*_mdl-{model_name}_sz_prob_forecast-{forecast}.pkl")
        #         model_paths[model_class][forecast] = out_path
        ...
        #         for forecast in all_forecasts:
        #             model_name = getattr(model_class, '__name__','UNKNOWN')
        #             out_path = model_paths[model_class][forecast]
        #             for opath in [out_path,out_path.replace('prob','mse')]:
        #                 sz_prob = pd.read_pickle(opath)
