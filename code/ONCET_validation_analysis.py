...
ONCET_THRESHOLD = 0.6  # Change this value as needed
...
def load_or_generate_oncet_probabilities(patient, onset_run, onset_labels, montage='bipolar', overwrite=False):
    ...
    checkpoint_path='/Users/wojemann/local_data/dynasd_data/PROCESSED_DATA/CHECKPOINTS/ONCET/balanced_xl/best_model.pth',
    config_path = '/Users/wojemann/local_data/dynasd_data/PROCESSED_DATA/CHECKPOINTS/ONCET/balanced_xl/config.json')
...
def main():
    ...
    roc_onset_path = ospj(figpath, 'ONCET_ROC_curve_onset.pdf')
    ...
    prc_onset_path = ospj(figpath, 'ONCET_PRC_curve_onset.pdf')
    ...
    # Add ONCET onset kappa values
    for kappa in all_onset_kappas:
        if not np.isnan(kappa):
            plot_data.append({'Type': 'ONCET', 'Phase': 'Onset', 'kappa': kappa})
    
    # Add ONCET spread kappa values
    for kappa in all_spread_kappas:
        if not np.isnan(kappa):
            plot_data.append({'Type': 'ONCET', 'Phase': 'Spread', 'kappa': kappa})
    ...
    if len(plot_data) > 0:
        ...
        kappa_path = ospj(figpath, 'ONCET_kappa_comparison.pdf')
    ...
    # Calculate seizure-level differences: Interrater - ONCET
    lme_data = []
    
    # Add onset data with differences
    for i, (patient, seizure_id, oncet_kappa, interrater_kappa) in enumerate(
        zip(all_patients_list, all_seizure_ids, all_onset_kappas, all_onset_interrater_kappas)
    ):
        if not np.isnan(oncet_kappa) and not np.isnan(interrater_kappa):
            difference = interrater_kappa - oncet_kappa
            lme_data.append({
                'patient': patient,
                'seizure_id': seizure_id,
                'phase': 'Onset',
                'difference': difference  # Interrater - ONCET
            })
    ...
    if len(lme_df[lme_df['phase'] == 'Onset']) > 0:
        ...
        try:
            ...
            # Intercept represents mean difference (Interrater - ONCET)
            ...
            # Extract statistics
            intercept_coef = result_onset.params['Intercept']  # Mean difference: Interrater - ONCET
            ...
            lme_results['LME_onset'] = {
                'mean_difference': float(intercept_coef),  # Interrater - ONCET
            ...
            print(f"  Mean difference (Interrater - ONCET): {intercept_coef:.4f} ± {intercept_se*1.96:.4f} (p={intercept_pval:.4f})")
    ...
    if len(lme_df[lme_df['phase'] == 'Spread']) > 0:
        ...
        try:
            ...
            # Extract statistics
            intercept_coef = result_spread.params['Intercept']  # Mean difference: Interrater - ONCET
    ...
    print(f"Kappa (ONCET Onset vs Consensus): {results['kappa_ONCET_onset_consensus']['mean']:.4f} ± {results['kappa_ONCET_onset_consensus']['SE']*1.96:.4f} (n={results['kappa_ONCET_onset_consensus']['n']}, threshold={ONCET_THRESHOLD})")
    print(f"Kappa (ONCET Spread vs Consensus): {results['kappa_ONCET_spread_consensus']['mean']:.4f} ± {results['kappa_ONCET_spread_consensus']['SE']*1.96:.4f} (n={results['kappa_ONCET_spread_consensus']['n']}, threshold={ONCET_THRESHOLD})")
